...
이 두 질문이 이 장을 관통하는 두 개의 큰 축입니다. 첫 번째 축("굶겨서 죽이기")은 §3~§5에서 유전자 필수성과 합성 치사로 다루고, 두 번째 축("고쳐서 되돌리기")은 §6~§7에서 MTA(Metabolic Transformation Algorithm)로 다룹니다. 두 축은 서로 배타적이지 않습니다 — 같은 대사 네트워크, 같은 화학량론 제약 $$\mathbf{S}\mathbf{v}=\mathbf{0}$$을 공유하면서, 다만 "무엇을 목적으로 삼는가"만 다릅니다. 마치 같은 지도를 놓고 어떤 사람은 "이 길을 막으면 적이 못 지나간다"를 찾고, 다른 사람은 "이 길을 열면 원래 가려던 목적지로 되돌아갈 수 있다"를 찾는 것과 비슷합니다.

[Chapter 6](../chapter-6/README.md)에서 우리는 RNA-seq 발현 데이터를 GEM에 통합해 **맥락 특이적 모델**을 만드는 방법을 배웠습니다. 그 결과물 — 암 세포주 모델, 대응 정상 조직 모델, source/target 전사체 쌍 — 이 이 장 전체의 원재료입니다. 이 장에서는 암 모델과 대응 정상 모델의 KO 효과 차이를 비교하고(§1~§5), MTA로 source 상태를 target이 시사하는 방향으로 옮길 후보를 순위화합니다(§6~§7). 어느 방법도 암 특이성·안전성·건강 회복을 자동 보장하지 않으므로 독립 실험과 정상 조직 패널 검증이 필요합니다.

{% hint style="info" %}
💡 **이 장의 지도.** §1은 "질병이란 무엇인가"를 대사 재프로그래밍의 언어로 재정의합니다(암·당뇨병·NAFLD·노화). §2는 §1에서 다룬 질병 모델이 실제로 어디서, 어떻게 만들어지는지(Ch5·Ch6)를 짧게 개관합니다. §3~§4는 "무엇을 끄면 죽는가"를 유전자 필수성과 합성 치사의 언어로 다룹니다. §5는 이 방법들을 실제 암종(간세포암·유방암·범암종 패널)에 적용한 사례 연구입니다. §6~§7은 관점을 뒤집어 "무엇을 조정하면 되돌아가는가"를 MTA·rMTA의 언어로 다루고, 암 이외의 질환(당뇨병·NAFLD·노화·COVID-19)까지 확장합니다. 즉 이 장은 "죽이기(§1,3~5) → 고치기(§1.5~6,7)"라는 하나의 큰 이야기 흐름을 가지고 있습니다.
{% endhint %}
