...
*그림 4.6: 파란 선은 반응별로 서로 다른 LP를 풀어 얻은 FVA 주변 구간이고, 주황 마름모는 그 안의 한 pFBA 대표 해이다(COBRApy 0.30.0, GLPK, `fraction_of_optimum=0.9`). 모든 파란 끝점을 동시에 갖는 단일 flux 벡터가 존재한다는 뜻이 아니며, 구간 폭은 통계적 신뢰구간도 아니다.*
...
표준 FVA는 화학량론과 플럭스 범위만 고려하므로, 환경과 물질을 교환하지 않고 ATP나 환원력을 무한 생성하는 열역학적으로 불가능한 순환(**제3형 경로, Type-III Pathway**, [Chapter 2](../chapter-2/README.md) 참고)을 허용할 수 있습니다. `loopless=True` 옵션(**Loopless FVA, ll-FBA**)은 이런 내부 순환을 추가 제약으로 제거합니다.
