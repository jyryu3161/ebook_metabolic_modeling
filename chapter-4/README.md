...
이 장은 처음부터 끝까지 반응 3개짜리 아주 작은 장난감 네트워크 하나를 반복해서 손으로 푼다 — 가능 영역을 오각형으로 그리고(3절), 심플렉스 타블로를 직접 계산하고(4절), 쌍대 문제로 그림자 가격을 유도하고(5절), pFBA로 무익한 순환을 제거하는(8절) 과정을 전부 **같은 숫자**로 검증한다. 이렇게 작은 예제에서 확인한 원리는 그대로 95차원의 `e_coli_core`, 나아가 수천 차원의 genome-scale 모델에도 적용된다 — 차원만 커질 뿐 이론은 바뀌지 않는다.
