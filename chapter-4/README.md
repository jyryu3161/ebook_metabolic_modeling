...
- **경계조건**(bounds), **목적함수**(objective function), **솔버**(solver)는 생물학적 사실이 아니라 분석자가 모델에 부여한 계산 조건이다.
...
[Chapter 2](../chapter-2/README.md)에서 대장균 코어 모델 `e_coli_core`(대사물 72개, 반응 95개)의 화학량론 행렬 $$\mathbf{S}$$와 정상상태 방정식 $$\mathbf{S}\mathbf{v}=\mathbf{0}$$을 구성했고, [Chapter 3](../chapter-3/README.md)에서 GPR 규칙, 구획, 바이오매스 반응을 더했다. 이 장이 다루는 계산 문제는 그렇게 구성된 모델에 배지와 경계조건을 지정했을 때 성장률에 대응하는 플럭스를 어떻게 계산하는가이다.

$$\mathbf{S}\mathbf{v}=\mathbf{0}$$과 플럭스 범위 제약만으로는 해가 하나로 정해지지 않는다. `e_coli_core`에는 독립 물질수지식보다 반응 변수가 많아, 이 제약을 만족하는 플럭스 벡터 $$\mathbf{v}$$가 무한히 많이 존재한다. 따라서 이 가능집합을 좁히려면 방정식과 부등식 외에 별도의 선택 기준을 지정해야 한다.

FBA는 세포의 관찰 상태가 특정 목적(대개는 빠른 성장)에 대해 최적이라는 가정을 하나 더 얹어 최적 목적값과 그 값을 달성하는 플럭스 해를 계산한다. 솔버는 한 벡터를 반환하지만 대안 최적해가 존재할 수 있다. 이 장은 그 계산 원리를 손 계산으로 검산하고 COBRApy로 실행하는 절차를 다룬다.
...
FBA는 세 가지를 함께 읽어야 한다. **물질수지**, **플럭스 경계**, **목적함수**다. 어느 하나라도 바뀌면 답도 달라질 수 있다.
