...
- COBRApy로 `e_coli_core`를 불러오고 NetworkX로 반응-반응 그래프를 만들어 연결 차수·매개 중심성 같은 위상 특징을 **추출합니다**.
- 야생형 FBA와 반응 결손 계산으로 각 반응의 플럭스 특징과 필수성 레이블을 **생성합니다**.
- scikit-learn의 Random Forest로 반응 필수성을 **학습**하고 ROC-AUC·MCC로 성능을 **평가합니다**.
- 서로 다른 배양 조건의 FBA 플럭스 벡터를 K-Means로 **클러스터링**하고 그 결과를 발효·호흡 축과 견주어 **해석합니다**.
- 각 단계가 이 장 어느 절의 개념을 재사용하는지 **연결합니다**.
