...
*그림 9.5. GEM을 GNN 예측기로 변환하는 일반 흐름. 화학량론 행렬과 선택한 flux 표현에서 반응 그래프·노드 특징·엣지 가중치를 만들고, 메시지 패싱으로 얻은 반응 임베딩을 예측 헤드에 입력합니다. 그래프 변환과 레이블 정의가 달라지면 같은 GAT 구조라도 답하는 질문이 달라집니다. 출처: 저자 자체 제작; 개념 근거: FlowGAT([Hasibi et al., 2024](https://doi.org/10.1038/s41540-024-00348-2))와 FluxGAT([Sharma et al., 2026](https://doi.org/10.1038/s41540-026-00738-8)). 두 논문의 그림은 복제하거나 변형하지 않았습니다.*
