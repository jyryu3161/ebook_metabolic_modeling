...
**[OptKnock](../landmark-papers.md)**(Burgard, Pharkya, Maranas, 2003)은 균주 설계의 시초격 알고리듬으로, 문제를 **이중 레벨 혼합정수선형계획법(bilevel MILP)**으로 정형화합니다. 상위 레벨(엔지니어)은 결손 조합(이진 변수 $$y_j$$)을 선택해 생산 flux $$v_{product}$$를 최대화하고, 하위 레벨(세포)은 그 제약 하에서 [FBA](../chapter-4/README.md)로 생장을 최대화한다고 가정합니다.
...
여기서 $$K$$는 허용 가능한 최대 결손 수입니다. 이 정형화의 핵심은 **strong duality(강 쌍대성)**를 이용해 이중 레벨 문제를 단일 레벨 MILP로 변환해 [Gurobi](https://www.gurobi.com/)·[CPLEX](https://www.ibm.com/products/ilog-cplex-optimization-studio) 같은 solver로 풀 수 있다는 점입니다. OptKnock은 생산과 생장의 결합을 **지향**하지만, 표준 optimistic bilevel 해는 최대 성장 대안해 가운데 생산이 큰 해를 선택할 수 있습니다. 따라서 같은 최대 성장률에서 산물 flux가 0인 대안해가 남는지 production envelope의 하한으로 확인해야 합니다.
...
### MUST set을 손으로 확인해 봅시다

OptForce의 첫 단계는 두 조건 — 참조(wild-type 성장 최적) 상태와 목표(과생산이 강제된) 상태 — 각각에서 FVA를 수행해 반응별 flux 허용 구간을 구하는 것입니다. 두 구간이 **겹치지 않으면** 그 반응의 flux는 목표를 달성하기 위해 반드시 바뀌어야 하는 반응, 즉 MUST set의 원소입니다. 겹치면 그 반응은 목표 달성에 굳이 조작하지 않아도 되는 반응입니다.

작은 예로 두 반응 $$r_A$$, $$r_B$$의 FVA 결과가 다음과 같다고 합시다(임의 수치).
...
$$r_A$$는 두 상태의 범위가 전혀 겹치지 않으므로($$5 < 8$$) 과생산을 달성하려면 flux가 반드시 참조 범위보다 커져야 합니다 — **MUSTU**(must up-regulate) 원소입니다. 같은 논리로 flux가 반드시 작아져야 하는 반응은 **MUSTL**(must down-regulate)에 속합니다. $$r_B$$는 두 범위가 $$[-3,6]$$에서 겹치므로 그 자체로는 변화가 강제되지 않습니다. 원 논문은 여기서 한 걸음 더 나아가 두 반응씩 묶은 MUSTUU·MUSTLL·MUSTUL까지 정의해, 단일 반응으로는 안 잡히지만 두 반응을 **함께** 조작해야 겹침이 깨지는 경우까지 찾습니다. MUST set은 "무엇이 바뀌어야 하는가"를 보여줄 뿐이며, 그중 실제로 유전적으로 조작 가능하고 개입 수가 최소인 부분집합을 고르는 다음 단계가 **FORCE set** 선정입니다.
...
BPCY는 생장률 $$\mu$$와 수율 $$Y_{P/S}$$(소비 기질당 산물 생산량)를 곱한 값으로, $$\text{BPCY} = \mu \times Y_{P/S}$$처럼 단순화해 이해할 수 있습니다. 생장만 보거나 수율만 보면 놓치는 트레이드오프를 하나의 숫자로 포착하려는 목적입니다.

두 결손 후보 균주를 비교해 봅시다(임의 수치).
