...
**아주 쉬운 비유부터 시작합시다.** 여러분의 집에서 회사까지 가는 길이 세 갈래(A, B, C)라고 합시다. 평소에는 어느 길로 가든 도착만 하면 되므로, "오늘은 어떤 길로 갈 수 있는가?"라는 질문의 답은 "A, B, C 모두"입니다. 그런데 길 B가 공사로 막혔다고 합시다. 이제 답은 "A 또는 C"로 줄어듭니다 — 없던 길이 새로 생기지는 않고, 갈 수 있는 선택지만 줄어듭니다. 세포에게 **유전자 결손**이란 정확히 이런 "도로 봉쇄"입니다. 이 장 전체가 이 단순한 그림 하나에서 출발합니다.
