...
**아주 쉬운 비유부터.** 한 사람이 자급자족하며 사는 것과, 여러 사람이 분업하며 사는 마을을 비교해 보십시오. 마을에서는 농부가 곡식을, 제빵사가 빵을, 대장장이가 연장을 만들며 서로 산물을 주고받습니다 — 어느 한 명만 떼어놓고 보면 결코 나타나지 않는 "마을 전체의 경제"가 생겨납니다. 미생물 세계도 똑같습니다. 한 종이 내놓은 부산물을 다른 종이 받아먹고(cross-feeding), 같은 먹이를 놓고 경쟁하며(competition), 서로 없이는 못 사는 관계(mutualism)를 맺습니다. 이런 "미생물 마을"의 대사는 종 하나짜리 모델로는 절대 재현할 수 없습니다.
...
**가중합 목적함수의 함정을 손으로 확인해 봅시다.** 두 종 A, B가 하나의 제한된 탄소원 pool을 나눠 쓰고, 존재비 가중치를 $$w_A=w_B=1$$로 두었다고 합시다. 탄소원이 부족해 두 종의 생장률 합이 $$\mu_A+\mu_B \le 1.0$$이라는 제약이 있다고 하면, 단순히 $$\max(\mu_A+\mu_B)$$만 풀면 다음 세 해가 모두 목적값 1.0으로 **동점**입니다.
...
순수 가중합 최대화는 해 1·2·3을 구분하지 못하므로, 솔버가 우연히 해 1을 반환하면 "커뮤니티 최적"이라는 이름으로 한 종을 멸종시키는 답을 내놓을 수 있습니다. 이것이 SteadyCom이 "모든 종이 **같은** 정상상태 커뮤니티 성장률을 공유"하도록 제약하거나, MICOM이 개별 종의 최소 성장률과 cooperative trade-off를 명시적으로 두는 이유입니다 — 즉 위 표에서 해 3만 남기는 추가 제약을 거는 것입니다.

> **잠깐, 생각해보기:** 장내 미생물 모델에서 존재비 가중치 $$w_k$$를 메타지놈 상대 존재비로 설정하는 것은 왜 중요할까요? 답: 실제로 많이 존재하는 종이 커뮤니티 대사에 더 크게 기여하도록 반영하기 위해서입니다. 그러나 $$w_k$$는 "그 종이 얼마나 많은가"이지 "그 종이 얼마나 빨리 자라는가"가 아니므로, 존재비가 큰 종이 반드시 높은 flux 기여를 갖는 것은 아닙니다 — MICOM은 이 둘(존재비와 성장)을 분리해 다룹니다.
