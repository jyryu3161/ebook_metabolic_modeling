...
*그림 8.3. Quadratic MOMA의 기하학. 보라색 야생형 기준점 $$(7,7)$$을 파란 돌연변이 feasible polytope에 유클리드 거리로 투영한 주황색 점 $$(4,4)$$이 MOMA 해이며, 초록색 별 $$(2,6)$$은 같은 공간의 성장 최대 FBA 해를 나타냅니다. 이는 임의 수치의 2차원 개념 모식도이지 `e_coli_core` 계산 결과가 아닙니다. 출처: 저자 자체 제작; 생성: [`scripts/generate_optimization_figures.py`](../scripts/generate_optimization_figures.py)의 `draw_moma_projection()`; 개념 근거: [Segrè et al. (2002)](https://doi.org/10.1073/pnas.232349399). MOMA 원 논문의 그림은 복제하거나 변형하지 않았습니다.*
