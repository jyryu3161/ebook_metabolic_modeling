...
| 반응 | $$w_i$$ | $$w_i^{\mathrm{up}}=w_i+0.1|w_i|+1$$ | $$w_i^{\mathrm{lo}}=w_i-0.1|w_i|-1$$ | 허용 구간 |
|:---:|:---:|:---:|:---:|:---:|
...
*그림 8.4. ROOM의 반응별 tolerance 판정. 파란 점과 띠는 야생형 flux $$w_i$$와 허용 구간, 마름모는 돌연변이 flux를 나타내며, 구간 안의 초록 마름모에는 $$y_i=0$$, 밖의 주황 마름모에는 $$y_i=1$$이 배정됩니다. ROOM은 거리의 합이 아니라 $$\sum_i y_i$$, 즉 허용 구간을 벗어난 반응 수를 최소화합니다. 임의 수치의 MILP 개념 모식도이며 COBRApy 계산 결과가 아닙니다. 출처: 저자 자체 제작; 생성: [`scripts/generate_optimization_figures.py`](../scripts/generate_optimization_figures.py)의 `draw_room_tolerance()`; 개념 근거: [Shlomi et al. (2005)](https://doi.org/10.1073/pnas.0406346102). ROOM 원 논문의 그림은 복제하거나 변형하지 않았습니다.*
...
*그림 8.5. ROOM MILP에 적용되는 branch-and-bound의 개념 흐름. 이진 변수의 LP relaxation에서 얻은 bound를 기준으로 $$y_k=0$$과 $$y_k=1$$인 자식 문제를 만들고, 가지치기와 분기를 반복해 정수해를 찾습니다. 실제 solver는 presolve·cut·휴리스틱과 별도의 node 선택 규칙을 함께 사용하므로 특정 solver의 실행 로그나 성능 측정은 아닙니다. 출처: 저자 자체 제작; ROOM 정형화의 개념 근거: [Shlomi et al. (2005)](https://doi.org/10.1073/pnas.0406346102). 원 논문의 그림은 복제하거나 변형하지 않았습니다.*
