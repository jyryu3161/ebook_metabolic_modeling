...
> 이 보강 단원은 Chapters 1–10의 핵심 개념을 연결한다. 모든 계산값은 **모델 파일·릴리스·배지·경계조건·목적함수·solver·허용오차**가 명시될 때에만 재현 가능한 결과가 된다. 아래의 도식과 예시는 교육용이며, 실험 관측 또는 임상적 효과의 증명이 아니다.
...
*그림 S.1. 재구축과 조건부 계산, 독립 검증의 관계. 저자 작성; 개념 근거: Thiele & Palsson (2010), Orth et al. (2010).*
...
*그림 S.2. FBA 종료 상태의 최소 진단 순서. 저자 작성; COBRApy 및 LP solver 문서의 일반 원칙을 교육용으로 종합.*
...
최소 release 패키지는 다음을 포함한다.

- SBML L3 FBC 모델 파일과 SHA-256, model release/tag
- 반응·대사물·GPR의 stable identifier와 MIRIAM/데이터베이스 주석
- 배지·수송 bounds·목적함수의 JSON/YAML 조건 파일
- solver·버전·tolerance·random seed를 고정한 실행 스크립트
- MEMOTE, mass/charge balance, flux consistency, task 및 phenotype benchmark 결과
- 변경 로그와 CI에서의 SBML round-trip 검사
...
```mermaid
flowchart LR
  RNA[RNA-seq] -->|RAS·반응 선택| G[context GEM]
  PROT[단백질체] -->|효소 용량 근거| G
  MET[대사체] -->|열역학·상태 근거| G
  EX[uptake/secretion] -->|bound·독립 검증| G
  MFA[13C-MFA] -->|내부 flux 검증| G
```

*그림 S.3. 오믹스 자료가 GEM에 주는 제약과 검증 역할은 동일하지 않다. 저자 작성.*
...
질병 맥락의 bulk·single-cell·spatial 자료는 서로 다른 세포 조성 및 공간 구배를 반영한다. 저산소, 영양 경쟁, lactate shuttle 및 약물 적응은 static FBA만으로 충분하지 않을 수 있으며, 필요한 경우 dFBA 또는 spatial/community 모델과 time-course 검증으로 확장한다.
...
*그림 S.4. 튜토리얼을 재현 가능한 연구 분석으로 확장하는 최소 산출물. 저자 작성.*
