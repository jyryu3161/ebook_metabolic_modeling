...
여기서 하한이 음수(-1000.0)이고 상한이 양수(1000.0)라는 것은, 제1장 §1.1에서 정의했듯이 저장된 반응식의 정·역방향 플럭스를 모두 허용한다는 뜻입니다. 음의 플럭스는 저장된 반응식의 역방향을 의미합니다. 그리고 `gene_reaction_rule`에 `b4025`라는 유전자 하나가 표시된 것은, 이 반응이 그 유전자 하나에 의해서만 촉매됨을 뜻합니다. 반응에 따라서는 여러 유전자가 `and`/`or`로 묶여 나타나기도 하는데, 이 논리 구조는 [Chapter 3](../chapter-3/README.md)에서 자세히 다룹니다. 지금은 “반응 객체 하나에 이름·반응식·플럭스 경계(bounds)·GPR이라는 네 가지 핵심 정보가 함께 들어 있다”는 것만 확인하고 넘어갑니다.
