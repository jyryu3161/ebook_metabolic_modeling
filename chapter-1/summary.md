...
- 대사는 물질, 자유에너지와 환원력을 획득·전환·배분·배출하는 화학 반응의 총체이다. GEM은 이 생화학 전체가 아니라 근거가 있는 대사·수송·경계·의사반응을 선택한 계산 표현이다.
- 대사물은 구분 가능한 화학종, 반응은 화학량론적 변환, 플럭스는 단위 시간당 반응 진행률이다. 플럭스와 대사물 농도, 화학량론 계수와 효소 활성은 서로 다른 양이다.
- 구조적 대체 경로는 잠재적 강건성을 제공하지만, 유전자 발현, 열역학, 보조인자 재생 및 효소 용량이 허용될 때만 실제 우회 플럭스를 형성한다.
- 네트워크의 degree와 essentiality는 동일하지 않으며, 통화 대사물의 포함과 그래프 투영법이 위상 지표에 큰 영향을 준다.
- 재구축은 반응·대사물·GPR·구획과 그 근거를 정리한 지식베이스이다. 여기에 배지, bounds, 대사 작업과 목적함수를 부여하면 특정 질문에 대한 조건부 계산 모델이 된다.
- 표준 FBA는 내부 대사물의 순축적을 무시하여 $$\mathbf S\mathbf v=\mathbf0$$을 적용한다. 이는 화학평형이나 영 플럭스를 뜻하지 않으며, 과도 상태와 저장물질 축적에는 별도의 동역학 표현이 필요하다.
- 제약 기반 모델은 가능한 정상 상태 집합을, 동역학 모델은 속도식과 초기조건 아래의 시간 궤적을 분석한다. ecGEM, ME-model, 오믹스 통합과 dynamic FBA는 서로 다른 정보를 추가하는 방법이며 하나의 우열 순서가 아니다.
...
4. iML1515의 모델 유전자 1,516개를 모두 독립 후보라고 단순화할 때 가능한 이중 결손 조합 수를 계산하시오. 실제 계산 스크리닝에서 이 수보다 후보가 줄거나 늘 수 있는 이유를 각각 하나씩 제시하시오.
...
| 의사-정상 상태 | 내부 대사물의 순축적을 무시하는 근사 | 화학평형과 구별해야 함 |
