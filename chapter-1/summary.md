...
- 네트워크의 연결 차수와 필수성은 동일하지 않으며, [통화 대사물](../glossary.md)의 포함과 그래프 투영법이 연결 구조 해석에 큰 영향을 준다.
...
- [제약 기반 모델](../glossary.md)은 정상상태에서 가능한 플럭스 범위를, [동역학 모델](../glossary.md)은 속도식과 초기조건 아래의 시간 궤적을 분석한다. [ecGEM](../glossary.md), ME-model, 오믹스 통합과 [dynamic FBA](../glossary.md)는 서로 다른 정보를 추가하는 방법이며 하나의 우열 순서가 아니다.
...
3. “정상상태이므로 모든 반응의 순플럭스가 0이다”라는 주장의 오류를 화학평형과 개방계 정상상태의 차이를 이용해 설명하시오.
...
7. “ATP는 대사 네트워크에서 연결 수가 가장 많은 대사물 중 하나이므로 ATP와 연결된 모든 반응은 필수이다”라는 주장을 비판하시오. 그래프 표현, 통화 대사물 및 조건부 필수성을 답에 포함하시오.
...
[Chapter 2](../chapter-2/README.md)는 반응식을 화학량론 행렬 $$\mathbf S$$로 옮기고, 각 내부 대사물의 생성과 소비가 맞는다는 $$\mathbf S\mathbf v=\mathbf0$$의 의미와 반응별 경계를 단계적으로 설명한다. Chapter 1에서 구분한 대사물·반응·플럭스의 단위와 모델 경계가 이후 모든 계산의 출발점이다.
...
| [의사-정상상태](../glossary.md) | 내부 대사물의 순축적을 무시하는 근사 | 화학평형과 구별해야 함 |
| FBA | 정상상태와 bounds 아래 선형 목적을 최적화하는 방법 | 목적함수 가설과 대안 최적해가 존재함 |
