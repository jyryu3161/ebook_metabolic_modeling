...
범용 [게놈 규모 대사 모델](../glossary.md)(Genome-Scale Metabolic Model, GEM)은 한 생물 또는 여러 인체 조직에서 알려진 대사 능력의 합집합을 나타냅니다. 특정 세포가 어느 반응에 실제로 flux를 갖는지는 전사체만으로 직접 관찰되지 않으며, 화학량론·배지·조절·효소량과 목적 가설에도 의존합니다. **[맥락 특이적 모델](../glossary.md)(context-specific model)**은 오믹스 증거와 기능 제약을 이용해 범용 GEM의 실행가능 공간을 줄이거나 서브네트워크를 추출한 계산 모델입니다. 따라서 실제 활성 네트워크의 직접 측정값이 아니라 검증해야 하는 가설입니다.
...
2. [GPR](../chapter-3/README.md)의 부울 구조와 연속 반응 점수로의 휴리스틱 변환
3. 발현 이산화, zFPKM과 [임계값](../glossary.md) 민감도
4. [GIMME](../landmark-papers.md)·[iMAT](../landmark-papers.md)·E-Flux·[tINIT](../landmark-papers.md)·[FASTCORE](../landmark-papers.md)의 최적화 정식화
5. RNA-seq 전처리와 generic/context 모델의 동일 조건 유전자 결손 비교
6. 효소·[열역학](../glossary.md) 제약과 다중 오믹스 통합의 한계
...
오믹스 자료는 대사 플럭스를 직접 대체하지 않는다. 이 장에서는 자료마다 GEM에 주는 정보의 층위가 다르다는 점을 먼저 잡는다.

1. RNA-seq은 반응 활성의 간접 근거이며 GPR을 통해 반응 점수로 변환된다.
2. 단백질체는 효소 용량의 근거가 될 수 있지만 $$k_{cat}$$·포화도·결측을 함께 고려한다.
3. 대사체는 상태와 열역학 해석을 보완하지만 농도와 플럭스를 혼동하지 않는다.
4. 교환 플럭스와 $$^{13}\mathrm C$$-MFA는 각각 경계와 내부 플럭스의 검증 자료로 활용된다.

{% hint style="warning" %}
발현 임계값(threshold)은 보편 상수가 아니다. 임계값, 알고리즘, 배지와 목적함수를 바꾼 감도 분석을 함께 제시한다.
{% endhint %}
...
2. **GPR(Gene-Protein-Reaction)**의 부울 AND/OR와 연속 발현값에 적용하는 min/max 휴리스틱을 구분하고 **[반응 활성 점수](../glossary.md)(Reaction Activity Score, RAS)**를 계산할 수 있다.
...
5. Raw counts로부터 **[TPM](../glossary.md)**을 계산하고, `e_coli_core`에 발현 기반 제약을 적용한 뒤 generic/context 모델의 유전자 결손 결과를 같은 기준으로 비교할 수 있다.
6. 단백질체 통합(**GECKO** [효소 제약](../glossary.md))과 대사체 통합(열역학·검출 기반 제약)의 단위와 가정을 설명할 수 있다.
