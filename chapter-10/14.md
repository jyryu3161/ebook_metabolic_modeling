...
버전 기록만으로는 충분하지 않습니다. 실제 연구에서는 Git commit, 입력 데이터 DOI/checksum, 모델 release, solver tolerance, 사용한 스크립트와 seed도 함께 기록합니다. `run_record["interpretation_policy"]`처럼 **숫자만이 아니라 그 숫자를 해석하는 규칙(임계값, 정책)까지 함께 저장**하는 습관은 몇 달 뒤 같은 파일을 다시 열어 보는 사람(원 작성자 포함)이 "7/24/106이라는 분류가 어떤 기준에서 나왔는지"를 코드를 다시 읽지 않고도 알 수 있게 해 줍니다.
