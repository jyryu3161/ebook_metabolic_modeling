...
세 방법 모두 같은 feasible space(같은 $$\mathbf{S}$$, 같은 bound) 안에서 답을 찾지만, **세포가 그 공간 안에서 어떤 해를 "선택한다"고 가정하는지**가 다릅니다. 자세한 수식 유도와 결과 해석의 함정은 [유전자 교란 보충 자료](../supplements/perturbation-analysis.md)에서 더 다룹니다. 이 절에서는 해당당분해효소(triose phosphate isomerase)를 인코딩하는 `tpiA`(b3919) 유전자를 결손시켜 세 방법의 예측을 나란히 비교합니다.
