...
지금까지 풀었던 문제(FBA, pFBA, FVA, 선형 MOMA/ROOM)는 모두 선형계획법(LP)이었습니다. LP의 변수는 연속값(continuous)이라서 "얼마나"는 표현할 수 있어도 "할 것인가 말 것인가"라는 이분법적 결정은 자연스럽게 표현하지 못합니다. 그런데 [Chapter 8](../chapter-8/README.md)의 OptKnock 같은 균주 설계 알고리듬이나 [§9](10.md)의 gap-filling은 본질적으로 "이 반응을 knockout 목록에 넣을 것인가", "이 후보 반응을 모델에 추가할 것인가"라는 **예/아니오 결정**을 다룹니다.
...
손으로 LP 행을 검산해 봅시다. 아래 코드의 LP는 $$x\ge0,\,y\ge0,\,x+y\le6,\,2x+y\le8$$이라는 다각형 안에서 $$3x+2y$$를 최대화합니다. 다각형의 꼭짓점 중 하나인 $$(2,4)$$를 대입하면 $$3\times2+2\times4=6+8=14$$로, 아래 코드에서 `solid` 실선으로 표시한 $$3x+2y=14$$ 등고선과 정확히 만납니다 — 실제로 LP의 최적해는 항상 다각형의 "꼭짓점"에서 나타난다는 [Chapter 4](../chapter-4/README.md)의 심플렉스법 원리를 이 작은 예제로 다시 확인할 수 있습니다.
...
*그림 10.2. LP·QP·MILP의 정적 개념 비교. (왼쪽) 선형 목적함수의 등고선이 feasible polygon의 꼭짓점 또는 최적 변에 닿는 LP, (가운데) 야생형 기준점을 돌연변이 feasible set에 투영하는 quadratic MOMA QP, (오른쪽) 야생형 tolerance 구간 이탈을 이진 변수로 세는 ROOM MILP입니다. 세 패널과 위 Plotly 예제는 문제 유형을 설명하는 임의 수치 모식도이며 서로 수치가 같은 실행 화면이나 실제 GEM 계산 결과가 아닙니다. 출처: 저자 자체 제작; 생성: [`scripts/generate_optimization_figures.py`](../scripts/generate_optimization_figures.py)의 `draw_lp_geometry()`, `draw_moma_projection()`, `draw_room_tolerance()`; 개념 근거: MOMA([Segrè et al., 2002](https://doi.org/10.1073/pnas.232349399))와 ROOM([Shlomi et al., 2005](https://doi.org/10.1073/pnas.0406346102)). 왼쪽 자산은 Chapter 4, 가운데와 오른쪽 자산은 각각 그림 8.3과 8.4에서도 동일 파일을 재사용하며, 원 논문의 그림은 복제하거나 변형하지 않았습니다.*
