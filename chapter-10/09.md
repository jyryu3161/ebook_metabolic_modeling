...
지금까지 풀었던 문제(FBA, pFBA, FVA, 선형 MOMA/ROOM)는 모두 선형계획법(LP)이었습니다. LP의 변수는 연속값(continuous)이라서 "얼마나"는 표현할 수 있어도 "할 것인가 말 것인가"라는 이분법적 결정은 자연스럽게 표현하지 못합니다. 그런데 [Chapter 8](../chapter-8/)의 OptKnock 같은 균주 설계 알고리듬이나 [§9](10.md)의 gap-filling은 본질적으로 "이 반응을 knockout 목록에 넣을 것인가", "이 후보 반응을 모델에 추가할 것인가"라는 **예/아니오 결정**을 다룹니다.

이런 결정을 수학적으로 표현하는 도구가 0 또는 1만 가질 수 있는 binary variable이고, 연속 변수와 binary variable을 함께 포함한 최적화 문제를 MILP(Mixed-Integer Linear Programming, 혼합정수계획법)라고 부릅니다. COBRApy는 이런 저수준 최적화 문제를 직접 조립할 때 `optlang`이라는 별도 라이브러리를 사용합니다(COBRApy가 `optlang` 위에 지어진 고수준 인터페이스라고 생각하면 됩니다). MILP·OptKnock의 이론적 배경은 [균주 설계 보충 자료](../supplements/perturbation-analysis.md)에서 더 다루므로, 이 절에서는 `optlang`으로 아주 작은 MILP를 직접 손으로 조립해 봅니다.
