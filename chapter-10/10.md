...
[Chapter 5](../chapter-5/README.md)에서 다루었듯, 게놈 규모 대사 모델을 재구축하는 과정에서 유전체 주석이 불완전하거나 특정 반응의 촉매 유전자가 아직 알려지지 않아 "대사경로 중간에 연결이 빠진" 상태가 자주 발생합니다. 예를 들어 대사물 A가 세포 안으로 들어오고 대사물 B가 biomass를 만드는 데 쓰이는데, A를 B로 바꿔 주는 반응이 모델에 없다면 biomass 생산 자체가 불가능(flux 0)해집니다. Gap-filling은 "universal reaction pool"(다른 생물종 모델이나 데이터베이스에서 가져온 후보 반응 목록)에서 최소한의 반응을 추가해 이 구멍을 메우는 알고리듬입니다.

내부적으로 gap-filling은 §8에서 배운 MILP와 같은 구조를 씁니다. 각 후보 반응마다 "이 반응을 추가하는가"를 나타내는 binary variable을 두고, 목표 flux(여기서는 biomass)가 최소 문턱값 이상이 되도록 제약을 건 뒤, **선택된 반응의 개수(또는 가중 비용)를 최소화**합니다. 즉 "구멍을 메우되, 가능한 한 적은 수의 반응만 추가하라"는 최적화 문제입니다.
