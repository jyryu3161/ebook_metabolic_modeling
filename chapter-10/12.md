...
*그림 10.4. glucose uptake cap에 따른 `e_coli_core` 최대 성장률의 산소 조건 의존성. 가로축은 glucose uptake cap($$\mathrm{mmol\,gDW^{-1}\,h^{-1}}$$), 세로축은 최대 biomass flux($$\mathrm{h}^{-1}$$)이며, 파란 선은 oxygen cap 1000, 주황 선은 cap 10에서 각각 0.5–20 범위를 40점으로 계산한 결과입니다. 출처: 저자 계산; 생성: [`scripts/generate_optimization_figures.py`](../scripts/generate_optimization_figures.py)의 `draw_robustness_curve()`; 조건: COBRApy 0.30.0 `textbook` 모델([BiGG `e_coli_core`](http://bigg.ucsd.edu/models/e_coli_core)), 기본 biomass 목적함수, GLPK(세부 버전 미기록), 단일 process. 아래 위젯은 임의의 glucose·oxygen 한 쌍에서 숫자 하나를 다시 계산하므로 이 두 곡선과 동일한 출력 화면은 아니며, Chapter 4의 robustness 절과 동일 PNG를 재사용합니다.*
