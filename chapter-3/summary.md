...
| 목적함수 | 선택한 반응과 가중치 | 가능한 플럭스 중 무엇을 최적화하는가? | 생물학적 목적을 가정하므로 실험 맥락과 검증이 필요하다. |

*표 S.1. GEM 구조 요소의 저장 위치, 해석 질문과 한계.*

## 1. 모델 크기는 생물학적 범위와 함께 읽는다

반응·대사물·유전자 수는 생물 종의 고정된 특성이 아니라 특정 모델 릴리스의 속성이다. 반응 수에는 내부 효소 반응뿐 아니라 운송·교환·요구·싱크·바이오매스 의사반응이 포함될 수 있고, 같은 화합물도 구획별로 여러 대사물 종으로 집계될 수 있다. 따라서 모델 크기만으로 생물학적 복잡성이나 예측 품질을 순위화하지 않는다.

모델을 비교할 때에는 릴리스, 구획별 대사물 집계 방식, 경계·바이오매스 반응 포함 여부와 GPR의 유전자 식별자 기준을 함께 기록한다. 연구 질문에 필요한 경로, 구획, 환경 경계와 근거 주석이 포함되었는지가 전체 숫자보다 중요한 구조 검사이다.

## 2. GPR은 유전자 기능 가용성의 논리식이다

효소 복합체의 필수 소단위는 AND, 같은 반응을 대신 촉매하는 동위효소는 OR로 표현한다.
...
단일 결손의 효과는 GPR 구조에 따라 다르다. `gA and gB`에서 `gA`를 결손하면 필수 소단위가 빠져 반응이 닫히지만, `gA or gB`에서는 `gB` 동위효소가 남아 반응이 구조적으로 가능하다. 혼합 규칙에서는 결손된 유전자가 속한 촉매 가지와 반응 전체의 Boolean 값을 따로 확인한다. 이 판정은 완전 기능상실을 가정하며 발현 보상, 부분 기능상실과 효소 용량을 직접 나타내지 않는다.
...
같은 화합물도 구획이 다르면 `pyr_c`와 `pyr_m`처럼 별도의 대사물 행으로 저장한다. `pyr_c`를 소비하고 `pyr_m`을 생성하는 운송 반응의 열에는 각각 −1과 +1이 기록된다. 구획 사이 이동은 자동으로 허용되지 않으며, 대응하는 운송 반응이 있어야 한다.

구획별 pH, 산화환원 상태와 보조인자 비율은 조건에 따라 달라지며, $$\mathbf{S}$$만으로 고정 농도비가 정해지지 않는다. 효소의 위치는 직접 실험, 큐레이션된 데이터베이스, 서열 신호와 예측 도구, 상동성 근거를 구분하여 기록한다. 위치 예측은 실험 근거가 부족할 때 후보 구획을 제시하는 보조 자료이며, 경로의 다른 반응과 운송 연결이 생물학적으로 일관되는지 함께 검토한다.
...
구획 수만으로 운송 반응 수를 계산할 수는 없다. 실제 수는 막의 연결성, 운반되는 대사물 종류, 수송체 기작, 방향과 모델 해상도에 따라 달라진다.
...
바이오매스 반응은 전구체와 에너지를 소비하는 $$\mathbf S$$의 열이다. 바이오매스 의사대사물은 선택적 표현이며 모든 모델에 존재하지 않는다. 목적함수는 별도의 계산 설정으로 최적화할 반응을 선택한다. 바이오매스 반응이 1 gDW 기준으로 정규화되고 균형 성장을 가정한 경우에만 그 플럭스를 특정 성장률 $$\mu\ [\mathrm{h^{-1}}]$$로 직접 해석한다.
...
1. 반응 수가 같은 두 모델의 생물학적 범위가 다를 수 있는 이유를 구획·경계·바이오매스 반응의 포함 기준으로 설명하시오.
2. `gA and gB`와 `gA or gB`에서 `gA` 단일 결손이 반응 가용성에 미치는 차이를 효소 복합체와 동위효소의 관점에서 설명하시오.
3. 빈 GPR을 가질 수 있는 생물학적·모델링 원인을 네 가지 제시하시오.
4. `pyr_c`를 소비하고 `pyr_m`을 생성하는 운송 반응을 쓰고, 두 대사물 행에 기록될 계수의 부호를 표시하시오.
5. 한 효소를 미토콘드리아 반응에 배정하기 위해 확인할 위치 실험·데이터베이스·서열 예측 근거를 구분하시오.
...
| [동위효소](02.md) | 서로 대체하여 같은 반응을 촉매할 수 있는 효소; 보통 OR로 표현 |
...
[Chapter 4](../chapter-4/README.md)에서는 이 장에서 구분한 $$\mathbf S$$, bounds와 목적함수를 선형계획 문제로 결합하여 FBA 해를 계산한다. GPR 단일 결손, 배지 변경과 BOF 선택이 각각 어떤 계산 조건의 변경으로 들어가는지 다룬다.
