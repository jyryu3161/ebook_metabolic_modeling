...
GEM의 규모와 구조는 생물종의 복잡성뿐 아니라 reconstruction scope, compartment definition, boundary convention 및 release history에 의해 결정된다. 따라서 ‘원핵 모델’과 ‘진핵 모델’의 보편적 반응 비율을 제시하기보다, 정확한 model release와 동일한 계산 정의로 비교해야 한다.
...
다음 표는 `cobra.io.load_model()`이 제공한 두 *E. coli* 모델을 동일 코드로 집계한 결과이다.
...
*Table 3.7: COBRApy 0.30.0의 `textbook`과 `iML1515` snapshot. Boundary는 `model.boundary`, transport proxy는 boundary가 아니면서 `len(reaction.compartments)>1`인 반응으로 정의했다. GPR은 COBRApy AST에서 AND·OR node를 검사해 상호 배타적으로 분류했다. 이 수치는 model release와 operational definition의 회귀 기준이다.*

`textbook` 모델에서 OR-only는 전체 반응의 $$27/95$$이지만 GPR이 있는 반응만 분모로 두면 $$27/69$$이다. 분모를 생략한 ‘OR 비율’은 비교할 수 없다. iML1515에서도 같은 이유로 651 OR-only reactions와 92 mixed reactions를 분리한다.
...
COBRApy 0.30.0 snapshot에서 `textbook` 모델의 `ATPM` lower bound는 8.39, iML1515는 6.86이며 단위는 모델 convention상 $$\mathrm{mmol\ ATP\ gDW^{-1}\ h^{-1}}$$이다. 이 값은 같은 생물종이라도 model release와 calibration에 따라 다를 수 있다. 다른 문헌의 NGAM을 모델 파일에 직접 대입하려면 배양 조건과 ATP maintenance 정의가 일치하는지 확인한다.
...
*Figure 3.3: GEM 구조 요소가 조건별 최적화 문제로 결합되는 과정. 저자 작성.*

FBA 입력은 ‘SBML 파일 하나’가 아니라 model release, objective, media, reaction bounds, maintenance 및 perturbation의 조합이다. 이 장의 구조 검사는 Chapter 4의 최적화 결과를 해석하기 위한 전제이며, 같은 구조에서도 조건이 달라지면 feasible set과 목적값이 달라진다.
