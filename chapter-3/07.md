...
![GEM 구조 검사의 순서](../.gitbook/assets/nano/ch3/ch3_fig13_model_anatomy_audit.jpg)

*그림 3.14. 조건별 분석 전에 수행하는 GEM 구조 검사 순서. GPR의 AND·OR, 구획별 대사물 위치, 막을 건너는 운송과 결합 물질, 배지의 경계 bounds, 바이오매스 조성과 유지 에너지를 차례로 확인하고 모델 릴리스·목적함수·허용오차를 함께 기록한다. 이 검사는 구조 누락과 계산 조건 누락을 구분하기 위한 절차이며 예측의 정확성을 자동으로 보장하지 않는다. 저자 작성 교육용 모식도이며 실제 모델 검증 점수나 계산 결과가 아니다. 개념 근거: [Thiele and Palsson (2010)](https://doi.org/10.1038/nprot.2009.203).*
...
*표 3.12. COBRApy 0.30.0의 `textbook`과 `iML1515` snapshot. Boundary는 `model.boundary`, transport proxy는 boundary가 아니면서 `len(reaction.compartments)>1`인 반응으로 정의했다. GPR은 [COBRApy](https://opencobra.github.io/cobrapy/) AST에서 AND·OR node를 검사해 상호 배타적으로 분류했다. 이 수치는 model release와 operational definition의 회귀 기준이다.*
