...
COBRApy 0.30.0의 `e_coli_core` 스냅샷에서 보고된 다음 값은 모델 bytes, loader, parser를 고정해 재실행할 때만 회귀값으로 비교한다.
...
이 지표들만으로 원핵생물과 진핵생물을 분류하거나 구조적 복잡도를 순위화할 수는 없다. 모델 범위, 구획화, GPR 큐레이션 규칙과 의사반응 수가 값에 함께 영향을 준다.
