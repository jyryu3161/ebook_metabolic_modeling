...
[화학량론 행렬](../chapter-2/README.md) $$\mathbf{S}\in\mathbb{R}^{m\times n}$$는 구획이 지정된 대사물과 반응 사이의 순 화학량론을 숫자로 기록한 표다. 그러나 실제 게놈 규모 대사 모델(genome-scale metabolic model, GEM)은 이 행렬 하나로 끝나지 않는다. 여기에 다음 구조와 메타데이터를 함께 결합한다.
