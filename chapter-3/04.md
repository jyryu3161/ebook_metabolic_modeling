...
따라서 해당 반응 열에는 extracellular species에 −1, cytosolic species에 +1이 기록된다. 이 단순 반응식만으로는 부족하다. 수송의 방향, 에너지 coupling, 그리고 transporter gene이 알려진 경우 그 정보까지 함께 추가해야 한다.
...
수송 반응의 방향성(가역성)을 어떻게 배정하는지는 다른 반응보다 특히 중요하다. 가역 수송을 너무 넉넉하게 허용하면 futile cycle이나 proton shuttle이 생긴다. 그러면 외부에서 기질을 공급하지 않아도 ATP나 이온 기울기가 만들어지고, 결국 성장률이 과대예측될 수 있다(§4.6의 7번 항목). 따라서 열역학·생리 근거가 없으면 수송 반응을 무조건 가역으로 두지 않는다. ATP 합성처럼 특정 방향만 알려진 결합은 그 방향으로 제한한다.
