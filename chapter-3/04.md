...
운송 반응은 한 구획의 metabolite species를 다른 구획의 species로 이동시킨다. 화학종이 변하지 않더라도 `glc__D_e`와 `glc__D_c`는 $$\mathbf S$$의 서로 다른 행이다.
...
여러 transporter가 같은 net substrate 이동을 촉매할 수 있으면 generic model은 OR GPR을 사용할 수 있다.
...
6. Exchange reaction과 intracellular transport를 혼동하지 않았는가?
