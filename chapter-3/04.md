...
[운송 반응](../glossary.md)은 한 [구획](03.md)의 대사물 종(metabolite species)을 다른 구획의 대사물 종으로 이동시킨다. 화학종이 변하지 않더라도 `glc__D_e`와 `glc__D_c`는 $$\mathbf S$$의 서로 다른 행이다.

$$
\mathrm{glc\_\_D_e}\rightleftharpoons\mathrm{glc\_\_D_c}
$$

따라서 해당 반응 열에는 extracellular species에 −1, cytosolic species에 +1이 기록된다. 이 단순 반응식만으로는 부족하다. 운송의 방향, 에너지 coupling, 그리고 transporter gene이 알려진 경우 그 정보까지 함께 추가해야 한다.
...
*그림 3.8. 단일수송(uniport), 공동수송(symport) 및 교환수송(antiport)의 화학량론적 방향. 이 분류는 수동·능동수송과 독립적인 축이며, 화살표는 특정 막전위·농도 조건에서의 실제 순 flux를 보장하지 않는다. 저자 작성 Mermaid 모식도이며 실제 모델 계산 결과가 아니다. 개념 근거: glucose uniport와 sodium–glucose symport의 생리 근거는 [Wright, Loo & Hirayama (2011)](https://doi.org/10.1152/physrev.00055.2009), mitochondrial carrier의 세 운반 mode는 [Ruprecht & Kunji (2020)](https://doi.org/10.1016/j.tibs.2019.11.001)을 참조했다. 원 논문 그림은 사용하지 않았다.*
...
*표 3.7. 대표 mitochondrial carriers. Substrate와 mode는 [Palmieri & Monné (2016)](https://doi.org/10.1016/j.bbamcr.2016.03.007) 및 [Ruprecht & Kunji (2020)](https://doi.org/10.1016/j.tibs.2019.11.001)을 바탕으로 요약했다.*
...
운송 반응의 방향성(가역성)을 어떻게 배정하는지는 다른 반응보다 특히 중요하다. 가역성을 너무 넉넉하게 허용하면 futile cycle이나 proton shuttle이 생긴다. 그러면 외부에서 기질을 공급하지 않아도 ATP나 이온 기울기가 만들어지고, 결국 성장률이 과대예측될 수 있다(§4.6의 7번 항목). 따라서 열역학·생리 근거가 없으면 운송 반응을 무조건 가역으로 두지 않는다. ATP 합성처럼 특정 방향만 알려진 결합은 그 방향으로 제한한다.

또한 기작이 확인되지 않아 (촉진)확산으로 가정한 세포 내 운송 반응은 실험 근거가 약하므로 대개 낮은 신뢰도(모델링 목적)로 기록한다. 재구축의 신뢰도 점수 체계와 가역성 검증 절차는 [Chapter 5](../chapter-5/README.md)에서 다룬다.
