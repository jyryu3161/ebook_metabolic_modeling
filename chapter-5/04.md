...
자동화 도구는 게놈 주석과 참조 지식을 입력받아 초안을 생성한다. 산출물은 반응 목록, [GPR](../chapter-3/README.md), 경계조건이다. 도구마다 출발 지식, 반응 선택 기준, biomass template, [gap-filling](../glossary.md) 정책이 다르다. 그래서 같은 게놈을 넣어도 서로 다른 모델이 나온다. 실행 시간이나 반응 수만으로는 품질을 비교할 수 없으며, 입력 DB release와 검증 조건을 함께 고정해야 한다.
...
_그림 5.8:_ 자동 재구축의 top-down과 bottom-up 전략. Top-down은 후보 반응 지식베이스를 게놈 근거로 줄이고, bottom-up은 기능 주석을 반응에 매핑해 초안을 쌓는다. 실제 도구는 두 전략의 요소를 함께 사용할 수 있으며 어느 출력도 검증된 최종 모델을 뜻하지 않는다. 실제 GEM 계산 결과가 아닌 교육용 Mermaid 모식도이다. 저자 작성; 개념 근거: [Machado et al. (2018)](https://doi.org/10.1093/nar/gky537), [Henry et al. (2010)](https://doi.org/10.1038/nbt.1672).
...
![후보 반응 지식베이스에서 서열 지지 초안과 gap-filling 가설이 포함된 배지별 초안으로 이어지며 DB release·입력 checksum·도구·배지가 기록되는 세 층.](../.gitbook/assets/nano/ch5/ch5_fig09_automated_draft_layers.jpg)
...
_표 5.4:_ 출판 당시 도구와 데이터 release를 사용한 통합 벤치마크. 효소 활성, 에너지원, 발효 생성물, 유전자 필수성 및 혐기성 먹이망 자료를 pooled하여 계산했으므로 특정 생물종이나 단일 endpoint의 보편 성능으로 해석하지 않는다. 출처: [Zimmermann et al. (2021)](https://doi.org/10.1186/s13059-021-02295-1), CC BY 4.0.
