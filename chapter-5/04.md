...
자동화 도구는 게놈 주석과 참조 지식으로부터 반응 목록, [GPR](../chapter-3/README.md) 및 경계 조건의 초안을 생성한다. 도구마다 출발 지식, 반응 선택 기준, biomass template 및 [gap-filling](../glossary.md) 정책이 다르므로 같은 게놈에서도 서로 다른 모델이 생성된다. 실행 시간이나 반응 수만으로 품질을 비교할 수 없으며, 입력 DB release와 검증 조건을 함께 고정해야 한다.
