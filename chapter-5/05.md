...
비슷해 보이지만 관찰 대상이 다른 세 용어를 구분한다. **dead-end 대사산물**은 한 대사물이 만들어지기만 하거나 소비되기만 하는 *증상*이다. **[차단 반응(blocked reaction)](../glossary.md)**은 현재 제약 아래 어떤 정상상태에서도 플럭스를 가질 수 없는 *반응 수준의 결과*다. **갭(gap)**은 이런 증상을 만들었을 가능성이 있는 *누락 반응 또는 기능*이라는 원인 가설이다. gap-filling은 원인 후보를 추가해 이 증상이 사라지는지 시험하는 절차다.
