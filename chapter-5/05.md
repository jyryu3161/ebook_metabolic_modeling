...
초안 모델이 지정한 배지에서 자라지 못하거나 알려진 대사 작업을 해내지 못할 때가 있다. 그렇다고 항상 반응이 빠진 것은 아니다. 교환 bound, biomass 조성, 반응 방향성, 대사산물 식별자, GPR도 같은 현상을 만들 수 있다. 이런 원인을 모두 점검한 뒤에도 기능 불일치가 남을 수 있다. **[Gap-filling](../glossary.md)**은 그 불일치를 설명하려고 후보 반응 집합에서 최소 비용의 추가 가설을 찾는 절차다.

Gap-filling 결과는 모델을 실행 가능하게 만드는 수학적 해이며, 대상 생물에 반응이 실제로 존재한다는 증명이 아니다. 추가 후보는 후속 서열 검색, 문헌 검토 및 표현형 실험의 우선순위로 취급한다.

여기서 세 개의 연결된 개념을 구분한다. **dead-end 대사산물**은 네트워크에서 생성만 되거나 소비만 되어 정상상태에서 균형을 맞출 수 없는 대사물로, 대사물 수준의 증상이다. **[blocked reaction](../glossary.md)**은 어떤 조건에서도 flux를 나를 수 없는 반응으로, [FVA](../glossary.md)로 검출하는 반응 수준의 증상이다. **갭(gap)**은 이 dead-end를 네트워크의 나머지와 연결해 증상을 해소할 **누락된 반응 또는 기능**, 즉 원인이다. gap-filling은 이 누락 반응의 후보를 제안하는 절차다.
