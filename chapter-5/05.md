...
초안 모델이 지정한 배지에서 자라지 못하거나 알려진 대사 작업을 해내지 못할 때가 있다. 그렇다고 항상 반응이 빠진 것은 아니다. 교환 bound, biomass 조성, 반응 방향성, 대사물 식별자, GPR도 같은 현상을 만들 수 있다. 이런 원인을 모두 점검한 뒤에도 기능 불일치가 남을 수 있다. **[Gap-filling](../glossary.md)**은 그 불일치를 설명하려고 후보 반응 집합에서 최소 비용의 추가 가설을 찾는 절차다.

Gap-filling 결과는 모델을 실행 가능하게 만드는 수학적 해이며, 대상 생물에 반응이 실제로 존재한다는 증명이 아니다. 추가 후보는 후속 서열 검색, 문헌 검토 및 표현형 실험의 우선순위로 취급한다.

비슷해 보이지만 관찰 대상이 다른 세 용어를 구분한다. **dead-end 대사물**은 한 대사물이 만들어지기만 하거나 소비되기만 하는 *증상*이다. **[차단 반응(blocked reaction)](../glossary.md)**은 현재 제약 아래 어떤 정상상태에서도 플럭스를 가질 수 없는 *반응 수준의 결과*다. **갭(gap)**은 이런 증상을 만들었을 가능성이 있는 *누락 반응 또는 기능*이라는 원인 가설이다. gap-filling은 원인 후보를 추가해 이 증상이 사라지는지 시험하는 절차다.
...
마지막 식은 최소 biomass flux, 특정 대사물 생산, ATP maintenance 또는 여러 배지의 성장과 같은 요구 기능을 나타낸다. 후보를 선택하지 않으면 $$y_r=0$$이므로 해당 flux도 0으로 고정된다. 이 표현은 임의의 단일 big-$$M$$ 대신 반응별 유한 bound $$\ell_r,u_r$$를 사용한다. 지나치게 큰 bound는 선형 완화와 수치 안정성을 악화시키므로 가능한 생리적·열역학적 근거로 제한한다.
...
![끊어진 A–D 경로에 두 반응짜리 비용 2 해와 한 반응짜리 비용 5 해가 제안되고 서열·문헌, 에너지 순환과 독립 표현형으로 검토되는 구조.](../.gitbook/assets/nano/ch5/ch5_fig11_gapfill_alternative_hypotheses.jpg)
...
- 반응을 추가하면서 새로 생긴 dead-end, 즉 **새 gap**(반응 추가 후 gap 분석을 다시 실행한다)
- 모델이 실제 생물에는 없는 기능을 얻는 **과잉 능력 부여** 여부

각 gap은 root gap과 downstream gap으로 구분해 기록한다. **root gap**은 특정 대사물의 생산 반응이 전혀 없거나(no-production) 소비 반응이 전혀 없는(no-consumption) 경우이고, **downstream gap**은 상류의 root gap 때문에 굶어 생긴 이차 증상으로 root gap을 채우면 대개 자동으로 해소된다. 따라서 downstream 증상부터 채우면 노력이 낭비된다. 직접 근거 없이 기능을 위해서만 추가한 반응은 confidence score 1(모델링 목적)로 표시해 이후 검색·재검토가 쉽도록 한다. gap을 채우는 일은 모델을 실제 생물보다 유능하게 만들 수 있으므로, 근거가 있거나 네트워크 기능에 반드시 필요할 때만 채운다.

## 5.3 대표 알고리즘의 목적 차이

| 방법 | 주된 목표 | 핵심 제한 |
|:---|:---|:---|
| SMILEY | 지정 대사물의 생산을 가능하게 하는 후보 반응 제안 | 목표와 candidate DB에 의존 |
| GapFind/GapFill | 생산·소비 불가능 대사물을 찾고 연결 가설 제안 | 연결 회복이 표현형 정확도를 보장하지 않음 |
...
_표 5.5:_ gap-filling 알고리즘은 서로 다른 ‘갭’을 목표로 한다. 출처: [Reed et al. (2006)](https://doi.org/10.1186/1471-2105-7-417), [Satish Kumar et al. (2007)](https://doi.org/10.1186/1471-2105-8-212), [Kumar and Maranas (2009)](https://doi.org/10.1371/journal.pcbi.1000308), [Thiele et al. (2014)](https://doi.org/10.1093/bioinformatics/btu321).
...
_그림 5.10:_ 학습 기반 점수와 제약 기반 gap-filling의 역할 분리. 학습 모델은 초안과 후보 데이터베이스에서 반응 검토 순위를 제안하고, 물리 제약을 포함한 gap-filling은 지정 기능을 만족하는 조합을 찾는다. 이후 유전자·문헌 근거와 독립 표현형을 검토하며, 어느 단계도 추가 반응의 존재를 단독으로 증명하지 않는다. 실제 GEM 계산 결과가 아닌 교육용 Mermaid 모식도이다. 저자 작성; 개념 근거: [Chen, Liao and Liu (2023)](https://doi.org/10.1038/s41467-023-38110-7). 원 논문 그림을 복제·변형하지 않았다.
...
| 자발 반응 | 효소 없이 진행 가능 | 관례상 인공 유전자 `s0001`/단백질 `S0001`을 배정해 orphan 반응과 구분하고, 비효소성 근거·방향성 기록 |
