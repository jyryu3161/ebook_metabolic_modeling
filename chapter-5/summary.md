...
1. **재구축과 모델은 구분된다.** 재구축은 반응·대사산물·GPR·구획·근거의 지식 표현이고, 모델은 여기에 조건별 bounds와 objective 또는 task를 부여한 계산 객체이다.
...
6. **Gap-filling은 누락 반응의 가설을 제안한다.** 최소 반응 수 또는 증거 비용을 최적화해 지정 task를 만족시키지만, 추가 반응의 생물학적 존재를 증명하지 않는다.
7. **모델 품질은 다차원적이다.** Mass balance, stoichiometric consistency, flux consistency, energy cycle, annotation 및 외부 phenotype accuracy는 서로 다른 속성을 측정한다.
8. **MEMOTE 총점은 설정에 의존한다.** 모델 release, MEMOTE version, solver, test selection 및 weight가 같을 때 회귀를 비교하고, 독립 표현형 검증을 별도로 수행한다.
9. **SBML package의 역할을 구분한다.** FBC는 bounds·objective·GPR 등을, Groups는 pathway·subsystem grouping을 표현한다.
10. **범용 인체 재구축은 평균 세포가 아니다.** 여러 인간 세포 유형에서 지지된 대사 지식의 통합 집합이며, 조직·세포 모델은 omics와 task를 이용해 별도로 추출한다.
11. **Human1과 Human2는 버전이 명시된 release로 인용한다.** Human2의 LLM은 GPR 후보를 선별했고, 전문가 검토·공개 provenance·자동 시험이 최종 큐레이션을 담당했다.
12. **tINIT은 evidence selection과 task preservation을 결합한다.** Task는 반응 하나의 포함 여부가 아니라 지정 입력·출력·bounds 아래 feasible flux state의 존재이다.
...
| Metabolic task | 지정 경계와 출력 조건을 만족하는 flux state의 존재로 정의한 기능 |
