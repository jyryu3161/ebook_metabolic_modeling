...
7. 모델 release를 재현 가능하게 인용하기 위한 metadata schema를 작성하라. 최소한 파일 checksum, DB release, solver, bounds, objective, task file 및 phenotype test split을 포함하라.
...
[Chapter 6](../chapter-6/README.md)은 인체 대사 재구축의 범위와 Human-GEM 계보·큐레이션을 소개한 뒤, 범용 재구축을 조직·세포 조건에 맞는 모델로 좁히는 방법을 다룬다. INIT 계열, GPR 기반 반응 점수, RNA-seq 전처리와 다중 오믹스 증거의 선택이 검증 결과에 미치는 영향을 비교한다.
