...
인체 대사 재구축도 미생물 GEM과 같은 보존 법칙과 증거 추적 원칙을 쓴다. 다만 적용 범위와 불확실성의 구조가 다르다. 하나의 인체 게놈은 여러 조직과 세포 유형이 공통으로 쓸 수 있는 대사 유전자의 기반을 담는다. 실제로 어떤 반응이 쓰이는지는 세포 유형, 분화 상태, 영양 환경, 질병, 시간에 따라 달라진다.

따라서 다음 두 객체를 구분한다.

- **범용 인체 재구축(generic human reconstruction)**: 인간에서 근거가 확인된 대사 반응의 통합 지식 기반이다. ‘평균 세포’의 활성 flux를 나타내지 않는다.
- **[맥락 특이 모델(context-specific model)](../glossary.md)**: 범용 재구축에 조직·세포·환경 자료와 경계조건을 적용해 특정 질문에 맞게 추출하거나 제약한 계산 모델이다.

## 7.1 미생물 모델과의 공통점과 차이

| 항목 | 미생물 GEM | 범용 인체 재구축 |
|:---|:---|:---|
| 공통 수학 구조 | $$\mathbf S\mathbf v=0$$, bounds, [GPR](../chapter-3/README.md), objective/task | 동일 |
...
| 대표 외부 검증 | 기질 이용, 성장, 분비, 결손 | [metabolic task](../glossary.md), biomarker, exchange, cell-line dependency |
...
*그림 5.7. 범용 인체 재구축에서 결합되는 증거 층. 저자 작성 모식도(모델 계산 결과 아님); 개념 근거: [Recon1](https://doi.org/10.1073/pnas.0610772104), [Human1](https://doi.org/10.1126/scisignal.aaz1482)의 재구축 절차.*

각 반응에는 다음 질문이 따른다.

1. 어떤 인간 유전자·transcript·protein isoform이 촉매 기능을 지지하는가?
2. 반응물, 생성물, 보조인자 및 방향성은 생리적 pH와 구획에서 타당한가?
3. 효소와 대사물이 같은 세포 [구획](../glossary.md)에 위치하는가?
...
인체 모델의 주요 release는 기존 재구축을 단순 합집합으로 병합하지 않는다. 서로 다른 식별자, protonation convention, compartment, reaction direction 및 GPR을 정규화한 뒤 중복과 충돌을 검토한다. Human1은 HMR2, iHsa 및 Recon3D를 통합하면서 반응·대사물 식별자와 GPR을 체계적으로 reconciliation한 사례이다.
...
- 게놈이 부호화한 반응이 모두 동시에 활성인 것은 아니다. 단세포 생물조차 게놈의 모든 반응을 동시에 사용하지 않으며, 인체에서는 조직 간 기능 분화와 소기관 구획화 때문에 이 간극이 훨씬 크다. 따라서 범용 재구축을 조건별 활성 상태와 동일시하지 않는다.
- [범용 인체 재구축](../glossary.md)은 여러 인간 세포 유형에서 지지된 반응의 통합 지식 기반이며, 어느 한 세포가 실제로 갖는 flux 상태의 평균이 아니다. ‘평균 세포’로 읽으면 통합 지식 기반과 조건별 활성 상태를 혼동하게 된다.
- biomarker 방향 예측, 세포주 dependency, task feasibility는 서로 다른 endpoint를 측정한다. 따라서 하나의 벤치마크 성능을 범용 재구축 전체의 ‘정확도’로 일반화하지 않는다.
- 서열 hit, metabolomics 검출, 데이터베이스 수록은 각각 하나의 증거 유형이다. top-down 또는 bottom-up 중 하나를 정답으로 두지 않고, 반응별 provenance를 유지하며 합의·충돌을 조정한다.
