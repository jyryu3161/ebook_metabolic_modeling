...
- **[맥락 특이 모델(context-specific model)](../glossary.md)**: 범용 재구축에 조직·세포·환경 자료와 경계 조건을 적용해 특정 질문에 맞게 추출하거나 제약한 계산 모델이다.
...
3. 효소와 대사산물이 같은 세포 [구획](../glossary.md)에 위치하는가?
...
- **“게놈에 있는 반응은 모두 쓰인다”**: 단세포 생물조차 게놈이 부호화한 모든 반응을 동시에 사용하지 않는다. 인체에서는 조직 간 기능 분화와 소기관 compartmentation 때문에 이 간극이 훨씬 크므로, 범용 재구축을 조건별 활성 상태와 동일시해서는 안 된다.
- **“범용 인체 재구축은 평균 세포다”**: [범용 인체 재구축](../glossary.md)은 여러 인간 세포 유형에서 지지된 반응의 통합 지식 기반이지, 어느 한 세포가 실제로 갖는 flux 상태의 평균이 아니다.
- **“한 벤치마크의 정확도가 전체 모델의 정확도다”**: biomarker 방향 예측, 세포주 dependency, task feasibility는 서로 다른 endpoint를 측정한다. 하나의 벤치마크 성능을 범용 재구축 전체의 ‘정확도’로 일반화할 수 없다.
- **“top-down 또는 bottom-up 중 하나가 정답이다”**: 서열 hit, metabolomics 검출, 데이터베이스 수록은 각각 하나의 증거 유형일 뿐이며, 현대 인체 재구축은 이를 단일 축으로 순위화하지 않고 반응별 provenance를 유지하며 합의·충돌을 조정한다.
