...
*표 11.3. 3-구성원 합성 군집의 MICOM 군집 solve 결과(solver Gurobi). 세 구성원이 공유 환경에서 균형 있게 성장합니다.*

![구성원 성장률과 군집 교환 프로파일](../.gitbook/assets/lab11/fig_cmig_community.png)

*그림 11.11. (왼쪽) 군집 내 세 구성원의 성장률(각 ≈0.437). (오른쪽) 군집 교환 프로파일: 포도당(−6.52)과 암모늄(−2.38)을 흡수하고 아세테이트(+5.15)·이산화탄소(+10.23)·물·양성자를 분비합니다. 음수=흡수, 양수=분비. 저자 계산·시각화; CMIG 0.1.0, MICOM 0.39.0, Gurobi.*
...
*그림 11.12. CMIG 동적 FBA. 포도당(빨강 점선)이 소모되며 바이오매스(초록)가 증가하다가, 포도당이 고갈되는 t≈5.36에서 성장이 멈춥니다. 저자 계산·시각화; CMIG 0.1.0, Gurobi, dt=0.1.*
...
*그림 11.13. CMIG 공간 미리보기. 왼쪽 경계에서 공급된 포도당이 확산하고 오른쪽에서 소비되어 농도 기울기가 생깁니다. 이러한 공간 기울기는 biofilm 군집(8장 사진 8.1)에서 위치에 따라 대사가 달라지는 이유를 보여 줍니다. 저자 계산·시각화; CMIG 0.1.0.*
