...
*그림 11.9. 같은 발현 증거로 CMM의 E-Flux2(가로축)와 LAD(세로축)가 예측한 반응별 flux 산점도. 대각선에서 벗어난 점은 두 통합 방법이 서로 다른 flux를 예측함을 보여 준다. 합성 발현을 사용했으므로 개별 수치가 아니라 방법 간 차이가 요점이다. 저자 계산·시각화; CMM 0.3.0, Gurobi.*
...
*그림 11.10. CMM 데스크톱 GUI. `e_coli_core`(반응 95·대사물 72·유전자 137, solver Gurobi)를 불러와 배지(acetate_aerobic)를 적용하고 FBA를 실행한 화면으로, 반응별 flux와 bound 편집, Comparison·Production·Omics·Flux Map 탭을 제공한다. 출처: CMM GUI, [github.com/jyryu3161/CMM](https://github.com/jyryu3161/CMM)(MIT).*
