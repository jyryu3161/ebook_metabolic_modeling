...
*표 11.1. mmplatform 데모의 tINIT 재구축·LAD 적합 결과. `method`가 `troppo_tinit`이면 정확한 커밋의 TROPPO tINIT이 실제로 실행된 것입니다.*

![표본별 LAD flux 적합 목적값](../.gitbook/assets/lab11/fig_mmplatform_ladfit.png)

*그림 11.7. mmplatform tINIT 데모의 표본별 LAD flux 적합 목적값(발현 편차 항). 참조 표본(회색)은 기준이므로 편차가 0에 가깝고, 질의 표본(빨강)은 발현 조건이 달라 편차가 큽니다. 저자 계산·시각화; mmplatform 0.3.0, TROPPO tINIT 경로, scipy-highs.*
