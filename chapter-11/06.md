...
**CMM(Cellular Metabolic Modeling Platform)**은 제약 기반 대사 모델링을 파이썬 API와 Qt 데스크톱 GUI로 함께 제공하는 도구입니다([CMM 공식 저장소](https://github.com/jyryu3161/CMM), MIT). FBA·pFBA·FVA, 발현 통합(LAD, 2단계 E-Flux2), 교란(MOMA·ROOM), 생산(FSEOF·FVSEOF), 균주 설계(OptKnock·RobustKnock), 정상화(MTA·rMTA)를 하나의 solver-중립 서비스로 묶습니다. 이 절은 [Chapter 6의 발현-제약 통합](../chapter-6/README.md)을 실제로 실행하며, 교란 방법의 가정은 [Chapter 4의 FBA·MOMA·ROOM 비교](../chapter-4/13.md)에 연결됩니다.
...
![E-Flux2 반응 flux를 가로축에, LAD 반응 flux를 세로축에 놓고 대각선과의 차이를 비교한 산점도](../.gitbook/assets/lab11/fig_cmm_eflux_lad.png)

_그림 11.12:_ 같은 발현 증거로 CMM의 E-Flux2(가로축)와 LAD(세로축)가 예측한 반응별 flux 산점도. 두 축 모두 반응 flux이며 단위는 `mmol gDW⁻¹ h⁻¹`입니다. 대각선에서 벗어난 점은 두 통합 방법이 서로 다른 flux를 예측함을 보여 줍니다. 합성 발현을 사용했으므로 개별 수치가 아니라 방법 간 차이가 요점입니다. 저자 계산·시각화; CMM 0.3.0, BiGG `e_coli_core`, 배지 `glucose_aerobic`, Gurobi.
...
![CMM 데스크톱 GUI에서 e_coli_core와 acetate_aerobic 배지를 선택하고 반응별 flux 표와 분석 탭을 표시한 화면](../.gitbook/assets/lab11/cmm_gui.png)

_그림 11.13:_ CMM 데스크톱 GUI. `e_coli_core`(반응 95·대사물 72·유전자 137, solver Gurobi)를 불러와 배지(acetate_aerobic)를 적용하고 FBA를 실행한 화면으로, 반응별 flux와 bound 편집, Comparison·Production·Omics·Flux Map 탭을 제공합니다. 출처: CMM GUI, [CMM 공식 저장소](https://github.com/jyryu3161/CMM)(MIT).
