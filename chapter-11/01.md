...
MOMA의 원형(2차) 목적함수나 대형 인체 모델의 tINIT 같은 큰 MILP는, QP와 대형 MILP를 빠르게 푸는 상용 solver에서 유리합니다. Gurobi는 학술 이메일로 무료 학술 라이선스를 제공합니다. 라이선스는 개인 계정에 귀속되므로 각자 발급해야 하며, 아래 절차는 사용자 환경에서 직접 실행합니다.

```bash
# 1) https://portal.gurobi.com 에서 학술 계정 등록 후
#    Named-User Academic 또는 WLS Academic 라이선스를 발급받는다.

# 2) gurobipy 설치(가상환경 안)
python -m pip install gurobipy

# 3) 라이선스 활성화 — Named-User는 grbgetkey가 필요하다.
#    (전체 Gurobi 설치 또는 conda 패키지 `gurobi`에 grbgetkey가 포함된다.)
#    대학 네트워크 또는 VPN에서 실행한다.
...
pip로 설치되는 `gurobipy`에는 크기 제한 체험판 라이선스가 포함되어, `e_coli_core` 같은 소형 모델의 QP·MILP는 별도 라이선스 없이도 실행됩니다. 그러나 Human-GEM 규모의 tINIT MILP는 정식 학술 라이선스가 있어야 제한에 걸리지 않습니다. 이 장의 2–5절은 GLPK와 체험판만으로 실행되며, 대형 인체 모델을 다루는 6절에서 정식 라이선스가 필요합니다.
