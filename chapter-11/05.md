...
`carve`는 내부적으로 (1) diamond로 입력 단백질을 universe의 유전자 서열에 정렬해 반응별 증거 점수를 매기고, (2) 혼합정수계획(MILP)으로 지지가 낮은 후보를 제거하면서 필요한 기능을 유지한 초안을 남깁니다. 결과 해석에는 MILP의 세부 식보다 어떤 서열 증거와 universe, solver를 사용했는지 기록하는 일이 중요합니다. 이 실행은 25,348개 반응을 후보로 하므로, GLPK보다 Gurobi·CPLEX 같은 solver에서 안정적으로 풀립니다([1절](01.md), [Chapter 5의 CarveMe 설명](../chapter-5/04.md)).
...
![e_coli_core·CarveMe 초안·iML1515의 반응·대사물·유전자 수를 모델별 세 막대로 비교한 그림](../.gitbook/assets/lab11/fig_carveme_compare.png)

_그림 11.10:_ 세 대장균 모델의 규모 비교. 교육용 `e_coli_core`(반응 95), 이 실습에서 자동 생성한 CarveMe 초안(반응 2,626), 수동 큐레이션한 iML1515(반응 2,712). 자동 초안이 게놈 규모 큐레이션 모델과 비슷한 규모에 도달하지만, 규모가 같다고 품질이 같다는 뜻은 아닙니다. 저자 계산·시각화; CarveMe 1.6.6, diamond 2.2.3, BiGG universe.
