...
대사 네트워크는 대사산물(metabolite)과 반응(reaction)이라는 두 요소로 표현한다. 대사산물은 반응에서 소비되거나 생성되는 물질(포도당, 피루브산 등)이다. 반응은 이런 물질들 사이의 화학량론적 변환이다. 반응은 대개 효소가 촉매한다. 다만 반응식·효소·유전자가 서로 일대일로 대응하지는 않는다. 하나의 반응을 여러 유전자가 함께 담당하는 복합체, 같은 반응을 맡는 여러 isoenzyme, 효소 없이 진행되는 자발 반응, 계산 편의를 위한 pseudo-reaction이 있기 때문이다. 그래서 유전자–단백질–반응 대응(GPR)의 상세 논리는 [Chapter 3](../chapter-3/README.md)에서 다룬다.
