...
큰 모델에서 눈을 돌려, 손으로 확인할 수 있는 아주 작은 네트워크로 $$\mathbf S\mathbf v$$의 의미를 검산합니다. [2.3절](02.md)의 닫힌(closed) 장난감 네트워크(반응 3개, 대사물 A·B·C)에서 모든 플럭스를 $$v_1=v_2=v_3=1$$로 두었을 때, 각 대사물의 순생성률 $$d\mathbf x/dt=\mathbf S\mathbf v$$를 계산합니다.
...
**확인 포인트**: 출력이 `dx/dt = [-2  0  2]`이면 성공입니다. A는 두 반응에서 소비되어 $$-2$$, B는 생성과 소비가 상쇄되어 $$0$$, C는 두 반응에서 생성되어 $$+2$$가 됩니다. B의 값이 0이라는 것은 이 플럭스 조합에서 B만 순축적이 없다는 뜻입니다.
